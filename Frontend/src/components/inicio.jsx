...
function Inicio({
  ...
  const notifiedHoneyMilestonesRef = useRef({})
  ...
  const currentEarnedHoneyPoints = useMemo(() => {
    ...
    return Math.floor(Math.max(0, elapsedSeconds) / FOCUS_SECONDS_PER_HONEY_POINT)
  }, [currentSeconds, timer])

  const liveHoneyPoints = honeyPoints + currentEarnedHoneyPoints

  useEffect(() => {
    if (!timer || timer.status !== "active") return

    const timerKey = String(timer.id)
    const previousMilestone = notifiedHoneyMilestonesRef.current[timerKey]

    if (previousMilestone === undefined) {
      notifiedHoneyMilestonesRef.current[timerKey] = currentEarnedHoneyPoints
      return
    }

    if (currentEarnedHoneyPoints > previousMilestone) {
      showHoneyToast(currentEarnedHoneyPoints - previousMilestone)
      notifiedHoneyMilestonesRef.current[timerKey] = currentEarnedHoneyPoints
    }
  }, [currentEarnedHoneyPoints, timer])
